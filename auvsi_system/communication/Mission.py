...
class Mission:
    ...
    def convertToWaypointMission(self):
        '''
        Accepts a list of waypoint dictionaries and returns a list of raw mission items
        '''

        MissionItems = []
        for seq, waypoint in enumerate(self.waypointList):
            latitude = int(waypoint["latitude"]*10**7)
            longitude = int(waypoint["longitude"]*10**7)
            altitude =  self.ConvertFootToMeter(waypoint["altitude"])
            if seq == 0:
                current_waypoint = self.vtolTakeoffWaypoint(seq, latitude, longitude, altitude)
            elif seq == len(self.waypointList) - 1:
                current_waypoint = self.vtolLandWaypoint(seq, latitude, longitude, altitude)
            else:
                current_waypoint = self.standardWaypoint(seq, latitude, longitude, altitude)
    
            MissionItems.append(current_waypoint)
            
        return MissionItems
    
    def convertToBoundaryPointPolygon(self):
        '''
    ...
    def getMissionWaypoints(self):
        ...
        self.missionWaypoints = self.convertToWaypointMission()
    ...
    def getMissionGeofence(self):
        ...
        self.missionGeofence = self.convertToBoundaryPointPolygon()
    ...
    def getMissionObstacles(self):
        ...
        self.missionObstacles = self.convertToObstaclePolygons()
