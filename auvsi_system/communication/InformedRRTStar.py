...
class InformedRRTStar:
    ...
    def __init__(self, start, goal,
                 obstacleList, boundary,
                 expandDis=0.0001, goalSampleRate=10, maxIter=400):

        ## Changed lat/lon input coordinates to x/y
        self.start = Node(start[1], start[0])
        self.goal = Node(goal[1], goal[0])

        ## Added conversion of input gps coordinates to output x/y coordinates
        self.boundary = boundary
        ## Added polygon boundary that points must stay inside
        self.free_space = path(self.boundary)

        ## Declare min/max rand area parameters
        self.min_rand_x = None
        self.max_rand_x = None
        self.min_rand_y = None
        self.max_rand_y = None
        self.calc_rand_area()
    ...
    ## Added function to calculate the rand area
    def calc_rand_area(self):

        min_rand_x = self.boundary[0][0]
        max_rand_x = self.boundary[0][0]
        min_rand_y = self.boundary[0][1]
        max_rand_y = self.boundary[0][1]
        ...
        print("Finished calculating rand area")

        self.min_rand_x = min_rand_x
        self.max_rand_x = max_rand_x
        self.min_rand_y = min_rand_y
        self.max_rand_y = max_rand_y
    ...
    def sample_free_space(self):
        ...
        ## Added do while loop to keep sampling points until one is within the 
        ## boundary polygon
        while True:
            ...
            ## Check to verify that point is in boundary polygon
            if (self.free_space.contains_point(rnd)):
                break
    ...
    def draw_graph(self, xCenter=None, cBest=None, cMin=None, e_theta=None,
        ...
        ## Added plot for boundary
        xs, ys = zip(*self.boundary)
        plt.plot(xs, ys, color='blue', marker='o', linestyle='solid', 
                    linewidth=2, markersize=2)

        ## Update plot axis to encompass whole rand area instead of just
        ## hard coded values
...
## To be deleted after permanent changes have been made
    # create obstacles
    #obstacleList = [
    #    (5, 5, 0.5),
    #    (9, 6, 1),
    ...
    ### create boundary point array
    ## Start off with easily modifiabe list of lists
    #boundary_list = [[-5, -5],[-6, 10],[5, 30],[20, 25],[30, 15],[30, 0]]
    ## append the first item to the end of the list to close the polygon
    #boundary_list.append(boundary_list[0])
   
    home = {'latitude': 38.144683, 'longitude': -76.427959, 'altitude': 8.0}

    current_mission = MissionPlan('text.json', home) 
    #waypoint_list = current_mission.getMissionWaypointArray()
    #obstacle_list = current_mission.getMissionObstacleArray()

    boundary_list = current_mission.getMissionGeofenceArray()
    boundary_list.append(boundary_list[0])
    obstacle_list = []

    # convert the list into a numpy array
    boundary_array = np.array(boundary_list, np.int32)
...
# To be deleted after permanent changes have been made
    # Set params
    rrt = InformedRRTStar(start=[38.144695, -76.427967], goal=[38.149652, -76.432947], 
                           boundary=boundary_list, obstacleList=obstacle_list)
